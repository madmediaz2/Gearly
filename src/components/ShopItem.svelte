...
<article class="border border-gray-200 rounded-lg overflow-hidden w-3xs flex flex-col bg-white shadow-md">
    <figure class="h-44 flex justify-center items-center p-4 bg-white">
        <img src={cartItem.image} alt={cartItem.name} class="max-w-full max-h-full object-contain" />
    </figure>

    <div class="p-3 px-4">
        <header class="flex flex-col gap-0.5">
			<p class="text-lg font-medium text-gray-800">{cartItem.name}</p>
			<h3 class="text-base font-normal text-gray-600 mb-3">{cartItem.brand}</h3>
		</header>
